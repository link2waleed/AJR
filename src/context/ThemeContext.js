...
export const ThemeProvider = ({ children }) => {
    ...
    const fetchPrayerData = useCallback(async (lat, lng) => {
        try {
            // Load school preference from storage
            const school = await StorageService.getSchoolPreference();
            const data = await PrayerTimeService.getCompletePrayerData(lat, lng, new Date(), school);
            if (data) {
                setPrayerData(data);
    ...
    const refreshTheme = useCallback(async () => {
        try {
            ...
            if (result.location) {
                ...
                // Load school preference for prayer times
                const school = await StorageService.getSchoolPreference();

                // Refresh all in parallel
                const [prayerResult, cityResult, weatherResult] = await Promise.all([
                    PrayerTimeService.getCompletePrayerData(lat, lng, new Date(), school),
                ...
                if (prayerResult) {
                    ...
                    setMaghribTime(prayerResult.maghribTime);
                }
    ...
    /**
     * Listen for school preference changes and refetch prayer times
     */
    useEffect(() => {
        const checkAndRefreshSchool = async () => {
            try {
                // Only refresh if we have prayer data and location
                if (prayerData && location) {
                    const school = await StorageService.getSchoolPreference();
                    
                    // Fetch latest prayer data with current school preference
                    ...
                    if (data) {
                        setPrayerData(data);
                        setMaghribTime(data.maghribTime);
                        console.log('ThemeContext: Prayer times updated for school change');
                    }
                }
            } catch (error) {
                console.error('ThemeContext: Error checking school preference:', error);
            }
        };

        // Check for school preference changes every 5 minutes 
        // This ensures dashboard updates when user changes school in PrayerTimesScreen
        const interval = setInterval(checkAndRefreshSchool, 5 * 60 * 1000);
